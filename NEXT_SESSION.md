...
## 🧭 Стратегическое направление: СБОР / формирование персональной карты (записано 2026-07-05)

**Тезис Rein'а: если нет карты — не над чем производить операции.** Весь каскад
(skeleton → personal → feedback), интерпретация, consolidation — всё бессмысленно
без наполненной персональной карты. Сбор — это входная воронка продукта, и её
надо развивать как самостоятельный аспект, а не как побочный эффект ingest'а vault'а.

Замысел (думать в эту сторону, детали прорабатывать):

1. **Нативные ненавязчивые тесты и опросники** — не «анкета на 40 вопросов»,
   а микро-взаимодействия, встроенные в естественный поток работы с Obsidian.
   Кандидаты: мини-вопрос при открытии заметки по её концептам; «это близко
   или далеко?» по паре концептов; выбор из 2-3 формулировок.
2. **Мягкие диалоги с Антуражем, инициированные им самим** — Антураж время от
   времени сам заводит разговор: спрашивает про малоосвещённые области карты,
   уточняет противоречия, предлагает поразмышлять. Триггеры: разреженные регионы
   карты, свежие концепты без связей, противоречия из FeedbackEngine.
   Важно: ненавязчиво — частота ограничена, легко отклонить, уважение к фокусу.
3. **Геймификация и награды** (возможно) — прогресс наполнения карты, стрики,
   визуальный рост карты как награда сама по себе, вехи («100 концептов»,
   «первая консолидация»). Осторожно: не превратить в дофаминовую дрочильню —
   награда должна отражать реальную ценность (карта стала полезнее).

Синергия с существующим: FeedbackEngine (#4 ниже) — уже готовый источник
вопросов для п.1-2; silent_pool (#6) — материал для затравок диалога; endpoints
`/feedback/questions` можно расширить до общего «источника взаимодействий».
Первый практический шаг: спроектировать «interaction engine» — единую очередь
взаимодействий (вопрос/диалог/награда) с приоритетами и rate-limit'ом, которую
плагин опрашивает, а слои каскада наполняют.
