...
| `S6` | Reject source-slot links that lack the explicitly requested relation | narrow typed relation-cue eligibility guard with local polarity/modality scope before deterministic slot rendering | unguarded `S4` planned-slot output on the same generation | promoted only for six registered English relations after public gates; unknown relations fail closed |
