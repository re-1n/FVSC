"""Leakage-controlled ablation for the explicit FVSC container hypothesis.

Every candidate sees the same earlier evidence and scores the same directed pairs in
later documents. The benchmark separates four questions:

* does a flat directed graph work;
* does the current density state work without explicit containers;
* does recursive asymmetric container structure work without density geometry;
* does projected density state add value on top of explicit containers.

The canonical append-only evidence ledger is not under test and remains unchanged.
"""
...
def _edge_rows(document: HeldoutDocument) -> list[tuple[str, str, float]]:
    return [
        ...
        for parent, child, weight, _subject_weight in _statement_rows(document.semantic_input)
    ]
...
class ContainerRepresentationSuite:
    ...
    @classmethod
    def fit(cls, documents: Sequence[HeldoutDocument]) -> "ContainerRepresentationSuite":
...
def _paired_document_bootstrap(
    ...
    *,
    ...
    for sample_index in range(samples):
        ...
        deltas[sample_index] = (
            (candidate_wins - baseline_wins) / comparisons if comparisons else 0.0
        )
    low, high = np.quantile(deltas, [0.025, 0.975])
    return float(low), float(high)


def run_container_bakeoff(
    ...
    *,
    ...
    suite = ContainerRepresentationSuite.fit(train)
    ...
    for document in test:
        ...
        document_outcomes.append(_DocumentOutcome(wins=document_wins, comparisons=comparisons))
        details.append({
            "source_id": document.source_id,
            "positives": len(all_edges),
            "known_positives": len(known_positives),
...
def evaluate_public_corpus(
    ...
    *,
    ...
    evaluation = run_container_bakeoff(
        documents,
