...
SLANG_DICT = {
    # -------------------------------------------------------------------
    # Аббревиатуры и сокращения — NORMALIZE
    # (форма сокращена, содержание не изменено)
    # Протокол: Feoktistov & Morozov 2025
    # -------------------------------------------------------------------
    "норм": "нормально",
    "нзч": "не за что",
    "спс": "спасибо",
    ...
    # -------------------------------------------------------------------
    # Разговорные формы — NORMALIZE
    # (фонетические сокращения стандартных слов)
    # -------------------------------------------------------------------
    "седня": "сегодня",
    "сёдня": "сегодня",
    "тож": "тоже",
    ...
    "нич": "ничего",
    # -------------------------------------------------------------------
    # Жаргон, мат, экспрессивная лексика — PRESERVE (не нормализуем)
    # Протокол: эти слова несут собственное семантическое содержание,
    # нормализация изменила бы смысл или регистр.
    # Удалены: бро, братан, бля, нах, нафиг, нахрен, пипец, капец,
    # фигня, херня, зашибись, збс, кайф, кайфово, топ, прикол,
    # прикольно, пофиг, пофигу, нифига, блин.
    # -------------------------------------------------------------------
}


# ---------------------------------------------------------------------------
# 1b. Known misspellings (direct replacement, checked before slang)
# ---------------------------------------------------------------------------
...
def normalize_text(text: str, fix_slang: bool = True,
    ...
    if fix_slang or fix_spelling:
        ...
        for word in words:
            ...
            # Step 0: Known misspellings (exact match, highest priority)
            if fix_slang and lower in KNOWN_MISSPELLINGS:
                replacement = KNOWN_MISSPELLINGS[lower]
