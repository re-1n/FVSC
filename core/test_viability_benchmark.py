...
def test_viability_benchmark_report_is_deterministic_and_bounded() -> None:
    ...
    assert report_a["benchmark"] == "fvsc-controlled-directionality-v5"
    ...
    for model_name in (
        ...
        assert 0.0 <= model["trace_matched_auc"] <= 1.0
        assert model["ranking_comparisons"] > 0
        assert model["trace_matched_comparisons"] >= 0
    ...
    trace_only = report_a["models"]["trace_mass_only"]
    assert normalized["accuracy"] == 0.5
    assert normalized["ranking_auc"] == 0.5
    if trace_only["trace_matched_comparisons"]:
        assert trace_only["trace_matched_auc"] == 0.5
