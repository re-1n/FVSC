"""Run the public baseline/claim-first consistency gate on local Ollama."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if output.exists():
        raise ValueError("refusing to overwrite an existing claim-first artifact")
    if not output.parent.is_dir():
        raise ValueError("claim-first artifact parent directory does not exist")
    ...
    generations = run_claim_first_pair(
        PUBLIC_CONSISTENCY_FIXTURES,
        backend_factory=backend_factory,
    )
    artifact = {
        "fixture_ids": [item.case_id for item in PUBLIC_CONSISTENCY_FIXTURES],
        "generations": [item.to_dict() for item in generations],
        "model_identity": identity.to_dict(),
        "num_ctx": args.num_ctx,
        "num_predict": args.num_predict,
        "prompt_version": CLAIM_FIRST_PROMPT_VERSION,
        "seed": args.seed,
        "temperature": 0.0,
    }
    output.write_text(
